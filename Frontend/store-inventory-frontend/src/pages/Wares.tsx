...
const Wares: React.FC = () => {
  const [wares, setWares] = useState<Ware[]>([]);
  const [loading, setLoading] = useState<boolean>(true);

  useEffect(() => {
    axios
      .get<Ware[]>('http://localhost:8000/api/wares/', { auth: { username: 'admin', password: 'seun@112' } })
      .then((response) => {
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl text-gray-700 mb-4">Wares</h2>
      {wares.length > 0 ? (
        ...
        <p className="text-center text-gray-600">No wares found.</p>
      )}
    </div>
  );
};
